  ...
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? 'max-md:hidden': ''}`}>
      ...
      <div className='px-5 text-xs'>
        <p>media</p>
        <div className='mt-2 max-h-[160px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {msgImages.map((url,ind) => (
            <div className='cursor-pointer rounded' key={ind} onClick={() => {window.open(url)}}>
